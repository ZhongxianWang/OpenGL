...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        camera.setProjectMatrix(projection);
...
void mouse_press_callback(GLFWwindow* window, int button, int action, int mods)
{
    if (g_camera)
    {
        ...
        g_camera->processMousePress(button, action, glm::vec2(xpos, ypos));
    }
}
...
void mouse_move_callback(GLFWwindow* window, double xpos, double ypos)
{
    if (g_camera)
    {
        int width, height;
        glfwGetWindowSize(window, &width, &height);
        g_camera->processMouseMove(glm::vec2(xpos, ypos), width, height);
    }
}
